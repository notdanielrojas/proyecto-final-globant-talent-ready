...
Call log:
  - waiting for locator('div[class=\'col-span-12 md:col-span-3\'] div[class=\'flex w-full space-x-2 mt-1\'] div:nth-child(1) input')
...
```ts
  ...
  23 |       "div[class='col-span-12 md:col-span-3'] div[class='flex w-full space-x-2 mt-1'] div:nth-child(1) input"
  24 |     );
  ...
  32 |     /* await this.selectMedioPagoOption(payment.mediosPago[0].medioOption); */
  33 |     await this.fillCuentaInput(payment.mediosPago[0].cuentaContable);
  34 |     await this.fillMontoInput(payment.mediosPago[0].monto);
  ...
  46 |   }
  47 | 
  48 | /*   async selectMedioPagoOption(medioOption: string): Promise<void> {
  49 |     await this.medioPagoSelect.selectOption({ label: medioOption });
  50 |   } */
  51 | 
...
> 53 |     await this.cuentaInput.fill(cuentaContable.toString());
  ...
  54 |   }
  55 | 
  56 |   async fillMontoInput(monto: number): Promise<void> {
  57 |     await this.montoInput.fill(monto.toString());
  58 |   }
  59 | 
  60 |   async clickGuardarCobranzaButton(): Promise<void> {
  61 |     await this.guardarCobranzaButton.click();
  62 |   }
  63 | }
```