...
```ts
  ...
  13 | 
  14 |     // Esperar la respuesta de la API en paralelo con el clic para evitar perder eventos de red
  15 |     await Promise.all([
  16 |       clientsPage.page.waitForResponse(
  17 |         (res) =>
  18 |           res.url().includes("/clients") &&
  19 |           (res.status() === 200 || res.status() === 201)
  20 |       ),
  21 |       clientsPage.guardarCambios(),
  22 |     ]);
  23 | 
  24 |     // Validar la visibilidad de la alerta con un margen de 10s
> 25 |     await expect(clientsPage.clientCreatedMessage).toBeVisible({ timeout: 10000 });
     |                                                    ^ Error: expect(locator).toBeVisible() failed
  26 |   });
  27 | });
```