...
```
Error: Timeout esperando la respuesta POST de creación de cliente
```
              ...
              - generic [ref=e142]:
                ...
                - generic [ref=e152]:
                  ...
                  - textbox "Nombre o Razón Social *" [ref=e155]:
                    ...
                    - text: Wuckert, Walker and Stracke
                - generic [ref=e156]:
...
```ts
  ...
  31 |     await this.cuitInput.fill(client.cuit);
  32 |     await this.razonSocialInput.fill(client.razonSocial);
  33 |     await this.emailInput.fill(client.email);
  34 |     await this.telefonoInput.fill(client.telefono);
  35 |     await this.domicilioInput.fill(client.domicilio);
  36 |     await this.localidadInput.fill(client.localidad);
  37 |   }
  38 | 
  39 |   // En ClientsPage.ts
  40 | 
  41 | async saveChangesAndGetClientCode(): Promise<CreatedClientResponse> {
  42 |   // 1. Definimos una promesa explícita para capturar la respuesta del POST de creación
  43 |   const clientResponsePromise = new Promise<any>((resolve, reject) => {
  44 |     const timeout = setTimeout(() => {
> 45 |       reject(new Error("Timeout esperando la respuesta POST de creación de cliente"));
     |              ^ Error: Timeout esperando la respuesta POST de creación de cliente
  46 |     }, 15000);
  47 | 
  48 |     const handler = async (response: any) => {
  49 |       const url = response.url();
  50 |       const status = response.status();
  51 |       const method = response.request().method();
  52 | 
  53 |       // Validar la URL completa del backend y que sea la respuesta del POST exitoso
  54 |       if (
  55 |         url.includes("back-imcoarca.leonardojose.dev/api/clients") &&
  56 |         method === "POST" &&
  57 |         (status === 200 || status === 201)
  58 |       ) {
  59 |         clearTimeout(timeout);
  60 |         this.page.removeListener("response", handler);
  61 |         try {
  ...
  68 |     };
  69 | 
  70 |     this.page.on("response", handler);
  71 |   });
  72 | 
  73 |   // 2. Hacer clic en Guardar Cambios
  74 |   await this.guardarCambiosButton.waitFor({ state: "visible" });
  75 |   await this.guardarCambiosButton.click();
  76 | 
  77 |   // 3. Obtener el payload parseado
  78 |   const responseJson = await clientResponsePromise;
  79 |   const data = responseJson.data || responseJson;
  80 | 
  81 |   return {
  82 |     id: Number(data.id),
  83 |     customerCode: data.customer_code || data.code,
  84 |   };
  85 | }
  86 | }
```