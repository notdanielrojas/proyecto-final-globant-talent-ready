              ...
              - generic [ref=e142]:
                ...
                - generic [ref=e152]:
                  ...
                  - textbox "Nombre o Razón Social *" [ref=e155]:
                    ...
                    - text: Becker, Labadie and Schimmel
                - generic [ref=e156]:
...
```ts
  ...
  13 | 
  14 |     // 1. Ejecutar el clic y esperar la respuesta de la API en paralelo
  15 |     await Promise.all([
> 16 |       clientsPage.page.waitForResponse(
     |                        ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  17 |         (res) => res.url().includes("/api/clients") && res.status() === 201
  18 |       ),
  19 |       clientsPage.guardarCambios(),
  20 |     ]);
  21 | 
  22 |     // 2. Esperar a que la redirección a /clientes se complete en la UI
  23 |     await expect(clientsPage.page).toHaveURL("https://imcoarca.leonardojose.dev/clientes", {
  24 |       timeout: 15000,
  25 |     });
  26 |   });
  27 | });
