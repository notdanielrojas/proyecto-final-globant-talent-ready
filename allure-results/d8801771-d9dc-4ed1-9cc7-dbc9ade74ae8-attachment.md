...
```ts
  ...
  13 | 
  14 |     // Guardar cambios directamente sin bloqueos de red manuables
  15 |     await clientsPage.guardarCambios();
  16 | 
  17 |     // Playwright reintentará automáticamente hasta que la SPA redirija a /clientes
> 18 |     await expect(clientsPage.page).toHaveURL("https://imcoarca.leonardojose.dev/clientes", {
     |                                    ^ Error: expect(page).toHaveURL(expected) failed
  19 |       timeout: 20000,
  20 |     });
  21 |   });
