              ...
              - generic [ref=e271]:
                ...
                - generic [ref=e312]:
                  ...
                  - textbox "Email" [ref=e315]:
                    - /placeholder: Uno o varios, separados por coma o espacio
                    - text: Doug.OConnell@hotmail.com
                - generic [ref=e316]:
...
```ts
  ...
  62 | 
  63 |  
  64 |  // En ClientsPage.ts
  65 | 
  66 | async guardarCambios() {
  67 |   // Esperar a que el botón esté visible y habilitado en el DOM
  68 |   await this.guardarCambiosButton.waitFor({ state: "visible" });
  69 |   await this.guardarCambiosButton.click();
  70 | }
  71 | 
  72 | async saveChangesAndGetClientCode(): Promise<CreatedClientResponse> {
  73 |   // 1. Preparar la escucha del evento de red
> 74 |   const responsePromise = this.page.waitForResponse(
     |                                     ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  75 |     (response) =>
  76 |       response.url().includes("/clients") &&
  77 |       response.request().method() === "POST" &&
  78 |       (response.status() === 200 || response.status() === 201),
  79 |     { timeout: 20000 } // Timeout expandido para Firefox/WebKit
  80 |   );
  81 | 
  82 |   // 2. Ejecutar la acción
  83 |   await this.guardarCambios();
  84 | 
  85 |   // 3. Esperar la respuesta
  86 |   const response = await responsePromise;
  87 |   const responseJson = await response.json();
  88 |   const clientData = responseJson.data || responseJson;
  89 | 
  90 |   return {
  91 |     id: Number(clientData.id),
  92 |     customerCode: clientData.customer_code || clientData.code,
  93 |   };
  94 | }
  95 | }
  96 | 
