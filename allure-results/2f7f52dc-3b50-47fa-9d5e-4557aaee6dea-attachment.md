              ...
              - generic [ref=e271]:
                ...
                - generic [ref=e312]:
                  ...
                  - textbox "Email" [ref=e315]:
                    - /placeholder: Uno o varios, separados por coma o espacio
                    - text: Cortney_Botsford@gmail.com
                - generic [ref=e316]:
...
```ts
  ...
  68 |   // 1. Escuchar el POST asignando un timeout propio de 15s a la espera de red
> 69 |   const responsePromise = this.page.waitForResponse(
     |                                     ^ TimeoutError: page.waitForResponse: Timeout 15000ms exceeded while waiting for event "response"
  70 |     (response) =>
  71 |       response.url().includes("/clients") &&
  72 |       response.request().method() === "POST" &&
  73 |       (response.status() === 200 || response.status() === 201),
  74 |     { timeout: 15000 }
  75 |   );
  76 | 
  77 |   // 2. Ejecutar el clic en el botón de guardar
  78 |   await this.saveChanges();
  79 | 
  80 |   // 3. Capturar y parsear la respuesta
  81 |   const response = await responsePromise;
  82 |   const responseJson = await response.json();
  83 |   const clientData = responseJson.data || responseJson;
  84 | 
  85 |   return {
  86 |     id: Number(clientData.id),
  87 |     customerCode: clientData.customer_code || clientData.code,
  88 |   };
  89 | }
  90 | }
  91 | 
