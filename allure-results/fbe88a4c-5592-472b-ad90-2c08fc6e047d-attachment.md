              ...
              - generic [ref=e271]:
                ...
                - generic [ref=e312]:
                  ...
                  - textbox "Email" [ref=e315]:
                    - /placeholder: Uno o varios, separados por coma o espacio
                    - text: Nat_Beier-Berge99@gmail.com
                - generic [ref=e316]:
...
```ts
  ...
  6  | }
  7  | 
  8  | export default class ClientsPage {
  9  |   readonly page: Page;
  10 |   readonly cuitInput: Locator;
  11 |   readonly razonSocialInput: Locator;
  12 |   readonly emailInput: Locator;
  13 |   readonly telefonoInput: Locator;
  14 |   readonly domicilioInput: Locator;
  15 |   readonly localidadInput: Locator;
  16 |   readonly guardarCambiosButton: Locator;
  17 | 
  18 |   constructor(page: Page) {
  19 |     this.page = page;
  20 |     this.cuitInput = page.getByRole("textbox", { name: "CUIT *" });
  21 |     this.razonSocialInput = page.getByRole("textbox", { name: "Nombre o Razón Social *" });
  22 |     this.emailInput = page.locator("#collection_email");
  23 |     this.telefonoInput = page.getByPlaceholder("Ej: 11-4444-5555");
  24 |     this.domicilioInput = page.getByRole("textbox", { name: "Domicilio Fiscal" });
  25 |     this.localidadInput = page.getByLabel("Localidad", { exact: true });
  26 |     this.guardarCambiosButton = page.getByRole("button", { name: "Guardar Cambios" });
  27 |   }
  28 | 
  29 |   async fillForm(client: ClientData): Promise<void> {
  30 |     await this.cuitInput.fill(client.cuit);
  31 |     await this.razonSocialInput.fill(client.razonSocial);
  32 |     await this.emailInput.fill(client.email);
  33 |     await this.telefonoInput.fill(client.telefono);
  34 |     await this.domicilioInput.fill(client.domicilio);
  35 |     await this.localidadInput.fill(client.localidad);
  36 |   }
  37 | 
  38 |   async saveChangesAndGetClientCode(): Promise<CreatedClientResponse> {
  39 |     // 1. Forzar el filtro estricto por método POST
  40 |     const [response] = await Promise.all([
> 41 |       this.page.waitForResponse(
     |                 ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  42 |         (res) =>
  43 |           res.url().includes("/clients") &&
  44 |           res.request().method().toUpperCase() === "POST" &&
  45 |           (res.status() === 200 || res.status() === 201)
  46 |       ),
  47 |       this.guardarCambiosButton.click(),
  48 |     ]);
  49 | 
  50 |     const responseJson = await response.json();
  51 |     const rawData = responseJson.data || responseJson;
  52 | 
  53 |     // 2. Extraer el objeto (si viene un array por el GET, tomamos el último creado)
  54 |     const clientData = Array.isArray(rawData)
  55 |       ? rawData[rawData.length - 1]
  56 |       : rawData;
  57 | 
  58 |     const code =
  59 |       clientData.customer_code ||
  60 |       clientData.customerCode ||
  61 |       clientData.code ||
  62 |       clientData.id?.toString();
  63 | 
  64 |     return {
  65 |       customerCode: code,
  66 |     };
  67 |   }
  68 | }
```