...
- Location: tests\createClient.spec.ts:7:3
...
Expected: "https://imcoarca.leonardojose.dev/clientes"
Received: "https://imcoarca.leonardojose.dev/clientes/nuevo"
...
- main:
  ...
  - textbox "Nombre o Razón Social *":
    ...
    - text: Witting, Smitham and Bailey
  - text: Retiene IVA
  ...
  - textbox "Email":
    ...
    - text: Alexandria_Dibbert@gmail.com
...
```ts
  ...
  3  | 
  4  | const CLIENT_URL = process.env.CLIENT_URL!;
  5  | 
  6  | test.describe("Client Management Test Suite", () => {
  7  |   test("User should be able to create a new client successfully", async ({ clientsPage, sidebar }) => {
  8  |     const clientData = ClientFactory.create();
  9  | 
  10 |     await sidebar.goToCreateClient();
  11 |     await clientsPage.fillForm(clientData);
  12 | 
  13 |     // Guardar cambios directamente sin bloqueos de red manuables
  14 |     await clientsPage.guardarCambios();
  15 | 
  16 |     // Playwright reintentará automáticamente hasta que la SPA redirija a /clientes
> 17 |     await expect(clientsPage.page).toHaveURL(CLIENT_URL, {
     |                                    ^ Error: expect(page).toHaveURL(expected) failed
  18 |       timeout: 20000,
  19 |     });
  20 |   });
