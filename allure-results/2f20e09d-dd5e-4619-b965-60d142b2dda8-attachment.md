              ...
              - generic [ref=e271]:
                ...
                - generic [ref=e312]:
                  ...
                  - textbox "Email" [ref=e315]:
                    - /placeholder: Uno o varios, separados por coma o espacio
                    - text: Tessie_Little86@hotmail.com
                - generic [ref=e316]:
...
```ts
  ...
  69 |     // 1. Interceptar la respuesta POST a /clients aceptando 200 y 201
> 70 |     const responsePromise = this.page.waitForResponse(
     |                                       ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  71 |       (response) =>
  72 |         response.url().includes("/clients") &&
  73 |         response.request().method() === "POST" &&
  74 |         (response.status() === 200 || response.status() === 201),
  75 |     );
  76 | 
  77 |     await this.saveChanges();
  78 | 
  79 |     const response = await responsePromise;
  80 |     const responseJson = await response.json();
  81 | 
  82 |     // 2. Manejar la respuesta tanto si viene envuelta en "data" como si viene directa
