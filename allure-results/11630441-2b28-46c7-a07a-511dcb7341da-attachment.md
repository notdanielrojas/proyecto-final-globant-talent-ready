              ...
              - generic [ref=e185]:
                ...
                - generic [ref=e210]:
                  ...
                  - textbox "Email" [ref=e213]:
                    ...
                    - text: Tremayne_Halvorson@gmail.com
                - generic [ref=e214]:
...
```ts
  ...
  15 | 
  16 |     // 1. Iniciar la escucha del POST específico antes de enviar
> 17 |     const responsePromise = page.waitForResponse(
     |                                  ^ Error: page.waitForResponse: Test timeout of 30000ms exceeded.
  18 |       (res) =>
  19 |         res.request().method() === "POST" &&
  20 |         (res.url().includes("/client") || res.url().includes("/cliente")) &&
  21 |         (res.status() === 200 || res.status() === 201)
  22 |     );
  23 | 
  24 |     // 2. Llenar y enviar el formulario
  25 |     await clientsPage.fillForm(clientData);
  26 | 
  27 |     // 3. Capturar la respuesta JSON del servidor
  28 |     const response = await responsePromise;
  29 |     const body = await response.json();
  30 | 
  31 |     // 4. Extraer el código asignado (priorizando 'codigo', luego 'id' o propiedades anidadas)
  32 |     createdClientCode = body.codigo || body.code || body.data?.codigo || body.id;
  33 | 
  34 |     await expect(clientsPage.page).toHaveURL(process.env.CLIENT_URL!, {
  35 |       timeout: 20000,
  36 |     });
  37 |   });
  38 | 
  39 |   test("2. Crear un nuevo Artículo", async ({ articlesPage, sidebar, page }) => {
  40 |     const articleData = ArticleFactory.create();
  41 | 
  42 |     await sidebar.goToCreateArticle();
  43 | 
  44 |     // 1. Escuchar la creación del producto/artículo
  45 |     const responsePromise = page.waitForResponse(
  46 |       (res) =>
  47 |         res.request().method() === "POST" &&
  48 |         (res.url().includes("/product") || res.url().includes("/articulo") || res.url().includes("/item")) &&
  49 |         (res.status() === 200 || res.status() === 201)
  50 |     );
  51 | 
  52 |     // 2. Llenar y enviar el formulario
  53 |     await articlesPage.fillForm(articleData);
  54 | 
  55 |     // 3. Esperar la respuesta HTTP
  56 |     const response = await responsePromise;
  57 |     const body = await response.json();
  58 | 
  59 |     // 4. Guardar el código real autogenerado
  60 |     createdArticleCode = body.codigo || body.code || body.data?.codigo || body.id;
  61 | 
  62 |     await expect(page).toHaveURL(process.env.ARTICLE_URL!, { timeout: 20000 });
  63 |   });
  64 | 
  65 |   test("3. Emitir una nueva Factura asociando Cliente y Artículo", async ({ invoicePage, sidebar, page }) => {
  66 |     const invoiceData = InvoiceFactory.create({
  67 |       clienteCodigo: createdClientCode, // Código real obtenido del backend (ej: "00173")
  68 |       items: [{ codigoArticulo: createdArticleCode }],
  69 |     });
  70 | 
  71 |     await sidebar.goToCreateInvoice();
  72 |     await invoicePage.fillForm(invoiceData);
  73 | 
  74 |     await expect(page).toHaveURL(process.env.INVOICE_URL!, {
  75 |       timeout: 20000,
  76 |     });
  77 |   });
  78 | 
  79 |   test("4. Registrar una nueva Cobranza", async ({ paymentPage, sidebar, page }) => {
  80 |     const paymentData = PaymentFactory.create({
  81 |       clienteCodigo: createdClientCode, // Mismo código real para buscar la factura emitida
  82 |     });
  83 | 
  84 |     await sidebar.goToCreatePayment();
  85 |     await paymentPage.fillForm(paymentData);
  86 | 
  87 |     await expect(page).toHaveURL(process.env.PAYMENT_URL!, {
  88 |       timeout: 20000,
  89 |     });
  90 |   });
  91 | });
