...
```ts
  ...
  18 |     this.agregarMedioDePagoButton = page.locator("//button[normalize-space()='Añadir Medio']//*[name()='svg']");
  19 |     this.medioPagoSelect = page.locator("select").first();
  20 |     this.cuentaInput = page.locator("div[class='col-span-12 md:col-span-3'] div[class='flex w-full space-x-2 mt-1'] div:nth-child(1)");
  21 |     this.montoInput = page.getByRole("textbox", { name: "0.00" });
  22 |     this.guardarCobranzaButton = page.getByRole("button", { name: "Guardar Cobranza" });
  23 |   }
  24 | 
  25 |   async fillForm(payment: PaymentData): Promise<void> {
  26 |     await this.clienteCodigoInput.fill(payment.clienteCodigo);
  27 |     await this.clienteCodigoInput.press("Enter");
  28 |     await this.page.waitForTimeout(1500);
  29 | 
  30 |     if (payment.fechaCobro) {
  31 |       await this.fechaCobroInput.fill(payment.fechaCobro);
  32 |     }
  33 | 
  34 |     const medio = payment.mediosPago?.[0];
  35 |     if (medio) {
> 36 |       await this.medioPagoSelect.selectOption({ label: medio.medioOption });
     |                                  ^ Error: locator.selectOption: Test timeout of 30000ms exceeded.
  37 |       await this.montoInput.fill(medio.monto.toString());
  38 |     }
  39 | 
  40 |     await this.guardarCobranzaButton.click();
  41 |   }
  42 | }
  43 | 
```