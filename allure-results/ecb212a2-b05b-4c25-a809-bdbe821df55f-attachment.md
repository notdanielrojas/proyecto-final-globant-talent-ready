...
Locator: getByText(/cliente guardado con éxito/i)
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByText(/cliente guardado con éxito/i)
...
```ts
  ...
  13 | 
  14 |     // 1. Escuchar la respuesta del backend
  ...
  19 |     );
  20 | 
  21 |     // 2. Guardar cambios
  22 |     await clientsPage.guardarCambios();
  23 |     await responsePromise;
  24 | 
  25 |     // 3. Validar la alerta / toast de éxito
  26 |     const alertMessage = clientsPage.page.getByText(/cliente guardado con éxito/i);
> 27 |     await expect(alertMessage).toBeVisible({ timeout: 10000 });
     |                                ^ Error: expect(locator).toBeVisible() failed
  28 |   });
  29 | });
```