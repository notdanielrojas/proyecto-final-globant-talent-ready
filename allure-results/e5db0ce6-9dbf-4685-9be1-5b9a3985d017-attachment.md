...
- main:
  ...
  - textbox "Email":
    ...
    - text: Mortimer_Harber@gmail.com
...
```ts
  ...
  13 | 
  14 |     // 1. Aceptar tanto status 201 (Created) como 200 (OK)
  15 |     await Promise.all([
  16 |       clientsPage.page.waitForResponse(
  17 |         (res) =>
  18 |           res.url().includes("/clients") &&
  19 |           (res.status() === 201 || res.status() === 200)
  20 |       ),
  21 |       clientsPage.guardarCambios(),
  22 |     ]);
  23 | 
  24 |     // 2. Validar la URL final a la que te redirige la SPA
> 25 |     await expect(clientsPage.page).toHaveURL("https://imcoarca.leonardojose.dev/clientes", {
     |                                    ^ Error: expect(page).toHaveURL(expected) failed
  26 |       timeout: 15000,
  27 |     });
  28 |   });
