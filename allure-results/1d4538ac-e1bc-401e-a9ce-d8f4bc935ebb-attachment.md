...
- main:
  ...
  - textbox "Nombre o Razón Social *":
    ...
    - text: Ledner, Schmidt and Leannon
  - text: Retiene IVA
  ...
  - textbox "Email":
    ...
    - text: Karson_Leuschke54@gmail.com
...
```ts
  ...
  13 | 
  14 |     // Esperar la respuesta de la API y presionar guardar
  ...
  22 |     ]);
  23 | 
  24 |     // Asertar directamente sobre el toast de Toastify
> 25 |     await expect(clientsPage.clientCreatedMessage).toBeVisible({ timeout: 10000 });
     |                                                    ^ Error: expect(locator).toBeVisible() failed
  26 |     await expect(clientsPage.clientCreatedMessage).toContainText("Cliente guardado con éxito");
  27 |   });
  28 | });
```