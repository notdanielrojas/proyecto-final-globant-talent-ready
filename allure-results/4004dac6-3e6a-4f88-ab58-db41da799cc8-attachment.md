...
- main:
  ...
  - textbox "Nombre o Razón Social *":
    ...
    - text: Ledner, Vandervort and Purdy
  - text: Retiene IVA
...
```ts
  ...
  13 | 
  14 |     // 1. Sincronizar el clic con la interceptación del POST en la red
  15 |     const responsePromise = clientsPage.page.waitForResponse(
  16 |       (res) =>
  17 |         res.url().includes("/clients") &&
  18 |         (res.status() === 200 || res.status() === 201)
  19 |     );
  20 |     await clientsPage.guardarCambios();
  21 |     await responsePromise; // Esperar a que la API procese la creación
  22 | 
  23 |     // 2. Darle margen al router de la SPA para cambiar a /clientes
> 24 |     await expect(clientsPage.page).toHaveURL("https://imcoarca.leonardojose.dev/clientes", {
     |                                    ^ Error: expect(page).toHaveURL(expected) failed
  25 |       timeout: 10000,
  26 |     });
  27 |   });
